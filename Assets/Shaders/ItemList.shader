Shader "ItemList"
 {
     Properties
     {
         ...
         _RadiusMin ("Radius Min", float) = 0
         _RadiusMax ("Radius Max", float) = 0
         _SubRadiusMin ("Sub Radius Min", float) = 0
         _SubRadiusMax ("Sub Radius Max", float) = 0
         ...
         SubShader
         {
             ...
             float4 MyFragmentShaderFunction(my_v2f  i) : COLOR
             {
                ...
                float dist = distance(_Center, i.pos);
                if(dist > _RadiusMin && dist < _RadiusMax) {
                    texcolor = fixed4(1, 1, 1, 1);

                }
                else if(dist > _SubRadiusMin && dist < _SubRadiusMax) {
                    texcolor = fixed4(0.8, 0.8, 0.8, 1);
